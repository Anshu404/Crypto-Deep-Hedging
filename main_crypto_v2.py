...
# ---------------------------------------------
# STRICTLY ISOLATED IMPORTS FOR CRYPTO V2
# ---------------------------------------------
from include.env_crypto_v2 import Env 
from testing_crypto_v2 import test_run
...
def main(model_name = None):
    ...
    print("\n" + "="*50)
    print(f'🚀 Training model {model_name} (STRICTLY CRYPTO V2 PIPELINE)')
    print("="*50 + "\n")
    ...
    # Original Multi-Layer Perceptron Actor Critic (No Transformer)
    ...
    for i in range(num_episodes):
        ...
        if i % settings['validation_interval'] == 0 and i > 0:
            ...
            # Saves models inside model/ folder
            actor_critic.save('model/' + model_name + '_' + str(i))
            
            a_rewards = 0
            b_rewards = 0
            
            # Using lists to accumulate DataFrames (safer for newer pandas versions)
            all_infos = []
            k = 0
            while k < set_count:
