...
def plot_performance(model_name):
    ...
    # 🛡️ THE ARRAY-BRACKET CLEANER (Graph crash hone se bachayega) 🛡️
    ...
    # ==========================================
    plt.subplot(1, 2, 1)
    sns.kdeplot(agent_pnl, color='blue', label='AI Agent (Transformer)', fill=True, alpha=0.3)
    ...
    bars = plt.bar(['AI Agent\n(Transformer)', 'Black-Scholes'], means, color=['blue', 'orange'], width=0.5)
    plt.title('Real Average Profit (Mean PnL)', fontsize=14, fontweight='bold')
    ...
    save_path = f"results/testing/{model_name}_Graph.png"
    plt.savefig(save_path, dpi=300)
    print(f"\n🚀 BOOM! Asli Graph save ho gaya hai yahan: {save_path}")
    
    # Screen par graph dikhane ke liye
    plt.show()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("👉 Sahi Command: python plot_transformer.py <model_name>")
        print("Example: python plot_transformer.py NSE_7000_results")
        sys.exit(1)
        
    model_input = sys.argv[1]
    plot_performance(model_input)