...
# ==========================================
# STRICT ISOLATION: Load the Indian Equity Environment V2!
# ==========================================
from include.env_nse_v2 import Env
...
def test_run(env, actor_critic, scaler, state_size, episode_number, empirical):
    ...
    while not done:
        ...
        info['cr1'] = 0
        info['cr2'] = 0
        info['ticker'] = getattr(env, 'ticker', 'NSE-V2-EQ')
...
def test_load(model_name):
    ...
    # THE FIX: Separate Real (DataKeeper) from NSE_V2 (Standalone)
    empirical = s['process'] == 'Real'
    is_nse = s['process'] == 'NSE_V2'
    ...
    if empirical:
        ...
        while not env.data_keeper.no_more_sets:
            stats, _, t_info = test_run(env, actor_critic, scaler, state_size, j, True)
            a_rewards += np.sum(stats['rewards'])
            b_rewards += np.sum(stats['b rewards'])
            print("\rEpisode {}/{}".format(j+1, set_count), end="")
            if info_df is None:
                info_df = t_info
            else:
        ...
        while j < sim:
            # Pass 'is_nse' so env_nse_v2 knows to trigger the real-world testing block!
            stats, _, t_info = test_run(env, actor_critic, scaler, state_size, j, is_nse)
            a_rewards += np.sum(stats['rewards'])
            b_rewards += np.sum(stats['b rewards'])
...
if __name__ == '__main__':
    ...
    if args['test']:
        print(f'Testing model {args["model"]} in NSE V2 Mode')
        test_load(args['model'])
        result_eval(args['model'])
    elif args['validate']:
        print(f'Validating model {args["model"]} in NSE V2 Mode')
        read_validation_files(args['model'])
    elif args['results']:
        print(f'Result eval model {args["model"]} in NSE V2 Mode')
        result_eval(args['model'])