...
def plot_nse_v2(model_name):
    ...
    print(f"📂 Loading REAL NSE V2 data from {file_path}...")
    ...
    sns.kdeplot(agent_pnl, color='blue', label='AI Agent (NSE V2)', fill=True, alpha=0.3)
    sns.kdeplot(bs_pnl, color='orange', label='Black-Scholes', fill=True, alpha=0.3)
    plt.title('NSE V2 PnL Distribution (Episode Level)', fontsize=14, fontweight='bold')
    ...
    bars = plt.bar(['AI Agent\n(NSE V2)', 'Black-Scholes'], means, color=['blue', 'orange'], width=0.5)
    plt.title('NSE V2 Average Profit (Mean)', fontsize=14, fontweight='bold')
    ...
    save_path = f"results/testing/{model_name}_NSE_V2_Graph.png"
    plt.savefig(save_path, dpi=300)
    print(f"\n🚀 BOOM! NSE V2 Graph save ho gaya hai yahan: {save_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("👉 Sahi Command: python plot_nse_v2.py NSE_V2_XXXX")
        sys.exit(1)
        
    model_input = sys.argv[1]
    plot_nse_v2(model_input)