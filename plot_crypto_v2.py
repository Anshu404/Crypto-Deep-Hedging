...
def plot_crypto_v2(model_name):
    ...
    print(f"📂 Loading REAL CRYPTO V2 data from {file_path}...")
    ...
    # ==========================================
    plt.subplot(1, 2, 1)
    # 🟢 V2 Labels updated here
    sns.kdeplot(agent_pnl, color='green', label='AI Agent (Crypto V2)', fill=True, alpha=0.3)
    sns.kdeplot(bs_pnl, color='orange', label='Black-Scholes', fill=True, alpha=0.3)
    plt.title('Crypto V2 PnL Distribution (Episode Level)', fontsize=14, fontweight='bold')
    plt.xlabel('Profit / Loss ($)', fontsize=12)
    plt.ylabel('Density', fontsize=12)
    
    # Adding vertical lines for Mean values
    plt.axvline(agent_pnl.mean(), color='green', linestyle='dashed', linewidth=1.5)
    ...
    bars = plt.bar(['AI Agent\n(Crypto V2)', 'Black-Scholes'], means, color=['green', 'orange'], width=0.5)
    plt.title('Crypto V2 Average Profit (Mean)', fontsize=14, fontweight='bold')
    ...
    save_path = f"results/testing/{model_name}_CryptoV2_Graph.png"
    plt.savefig(save_path, dpi=300)
    print(f"\n🚀 BOOM! Crypto V2 Graph save ho gaya hai yahan: {save_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("👉 Sahi Command: python plot_crypto_v2.py CryptoV2_XXXX")
        sys.exit(1)
        
    model_input = sys.argv[1]
    plot_crypto_v2(model_input)