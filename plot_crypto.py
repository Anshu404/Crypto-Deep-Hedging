...
def plot_crypto(model_name):
    ...
    print(f"📂 Loading REAL CRYPTO data from {file_path}...")
    ...
    # ==========================================
    plt.subplot(1, 2, 1)
    sns.kdeplot(agent_pnl, color='green', label='AI Agent (Crypto TD3)', fill=True, alpha=0.3)
    sns.kdeplot(bs_pnl, color='orange', label='Black-Scholes', fill=True, alpha=0.3)
    plt.title('Crypto PnL Distribution (Episode Level)', fontsize=14, fontweight='bold')
    ...
    bars = plt.bar(['AI Agent\n(Crypto TD3)', 'Black-Scholes'], means, color=['green', 'orange'], width=0.5)
    plt.title('Crypto Average Profit (Mean)', fontsize=14, fontweight='bold')
    ...
    save_path = f"results/testing/{model_name}_Crypto_Graph.png"
    plt.savefig(save_path, dpi=300)
    print(f"\n🚀 BOOM! Crypto Graph save ho gaya hai yahan: {save_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("👉 Sahi Command: python plot_crypto.py Crypto_XXXX_results")
        sys.exit(1)
        
    model_input = sys.argv[1]
    plot_crypto(model_input)